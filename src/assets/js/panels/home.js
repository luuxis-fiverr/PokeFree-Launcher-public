...
class Home {
    ...
    goToWebSite() {
        let socials = document.querySelectorAll('.server-status-icon')
...
async loadNews() {
    try {
        const res = await fetch("https://pokefree.fr/api/rss");
        ...
        items.forEach((item, i) => {
            ...
            const titleFull = item.querySelector("title")?.textContent?.trim() || "News sans titre";
            const link = item.querySelector("link")?.textContent?.trim();
            const maxLength = 60;
            const titleShort = titleFull.length > maxLength 
                ? titleFull.substring(0, maxLength - 3) + "..." 
                : titleFull;

            const li = document.createElement("li");
            li.classList.add("news-item");
            li.textContent = titleShort;
            li.setAttribute("title", titleFull);
            if (link) {
